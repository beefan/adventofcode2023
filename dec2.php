...
class Bag
{
  ...
  public function __construct(int $redStart, int $greenStart, int $blueStart)
  {
    $this->counts['red'] = $redStart;
    $this->counts['blue'] = $blueStart;
    $this->counts['green'] = $greenStart;
  }

  public function pickCubes(string $color, int $count)
  {
...
class Game
{
  ...
  {
    $split = explode(':', $line);
    $this->id = (int) explode(' ', trim($split[0]))[1];

    $this->steps = array_map(function ($item) {
      $cubePicks = explode(', ', trim($item));
    ...
    }, explode(';', trim($split[1])));

    $this->bag = $this->newBag();
  }

  private function newBag(): Bag
  {
    return new Bag(self::REDSTART, self::GREENSTART, self::BLUESTART);
  }
  ...
  public function play(): bool
  {
    foreach ($this->steps as $step) {
      foreach ($step as $subStep) {
        $this->bag->pickCubes($subStep['color'], $subStep['count']);
      }

      if (!$this->bag->isValidBag()) return false;
...
function advent()
{
  ...
  foreach ($file as $line) {
    $game = new Game($line);
    if ($game->play()) $success[] = $game->id;
  }

  var_dump(array_reduce($success, fn ($acc, $id) => $acc + $id));
}
