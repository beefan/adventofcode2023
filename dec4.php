<article class="day-desc">
  ...
  <p>As far as the Elf has been able to figure out, you have to figure out which of the <em>numbers you have</em> appear in the list of <em>winning numbers</em>.
    The first match makes the card worth <em>one point</em> and each match after the first <em>doubles</em> the point value of that card.</p>
  <p>For example:</p>
...
class Card
{
  ...
  private function winners(): array
  {
    ...
    foreach ($this->pickNumbers as $pick) {
      if (in_array($pick, $this->winningNumbers)) {
        $count = count(
          array_filter($this->pickNumbers, fn ($n) => $n === $pick)
        );
        $matches[$pick] = $count;
      }
    }
