...
class Almanac
{
  ...
  {
    $seedPrep = explode('seeds: ', $input[0])[1];
    $this->seeds = explode(' ', $seedPrep);

    $this->seedToSoil = new Map($this->parseMap($input[1]));
    $this->soilToFertilizer = new Map($this->parseMap($input[2]));
    $this->fertilizerToWater = new Map($this->parseMap($input[3]));
    $this->waterToLight = new Map($this->parseMap($input[4]));
    $this->lightToTemperature = new Map($this->parseMap($input[5]));
    $this->temperatureToHumidity = new Map($this->parseMap($input[6]));
    $this->humidityToLocation = new Map($this->parseMap($input[7]));
  }
  ...
  public function getLocationNumbers(): array
  {
    ...
    foreach ($this->seeds as $seed) {
      $soil = $this->seedToSoil->get($seed);
      $fert = $this->soilToFertilizer->get($soil);
      $water = $this->fertilizerToWater->get($fert);
      $light = $this->waterToLight->get($water);
      $temp = $this->lightToTemperature->get($light);
      $humid = $this->temperatureToHumidity->get($temp);
      $loc = $this->humidityToLocation->get($humid);
      $locations[] = $loc;
    }
  ...
  private function parseMap($string): array
  {
    $explody = explode(PHP_EOL, $string);
    return [
      'type' => explode(' ', $explody[0])[0],
      'input' => array_slice($explody, 1),
    ];
  }
}
...
class Map
{
  ...
  public function __construct(array $input)
  {
    $splody = explode('-to-', $input['type']);

    $this->source = $splody[0];
    $this->destination = $splody[1];

    foreach ($input['input'] as $line) {
      $this->inputs[] = explode(' ', $line);
    }
  }
  ...
  public function get(int $key): int
  {
    foreach ($this->inputs as $input) {
      [$destinationStart, $sourceStart, $range] = $input;

      $inRange =
        $key >= $sourceStart &&
        $key <= $sourceStart + $range;

      if ($inRange) {
        $diff =  $key - $sourceStart;
        return $diff + $destinationStart;
      }
    }
...
function advent()
{
  $input = file_get_contents('dec5.input');
  // $input = <<<TEXT
  //   seeds: 79 14 55 13
  ...
  $almanac = new Almanac(explode(PHP_EOL . PHP_EOL, $input));
  $locations = $almanac->getLocationNumbers();
